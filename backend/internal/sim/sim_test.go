...
func TestTickDeterminismSameSeed(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		e1.Tick(s1)
		e2.Tick(s2)
		if s1.Day != s2.Day || s1.Cash != s2.Cash {
			t.Fatalf("divergence at tick %d: s1={day:%d cash:%d} s2={day:%d cash:%d}",
				i+1, s1.Day, s1.Cash, s2.Day, s2.Cash)
		}
	}
}
...
func TestTickDeltaInRange(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		prev := state.Cash
		e.Tick(state)
		delta := state.Cash - prev
		if delta < MinDailyDeltaCents || delta > MaxDailyDeltaCents {
			t.Fatalf("tick %d delta = %d, out of [%d, %d]", i+1, delta, MinDailyDeltaCents, MaxDailyDeltaCents)
		}
	}
}
...
func TestAdvanceDays(t *testing.T) {
	...
	if state.Day != ref.Day || state.Cash != ref.Cash {
		t.Errorf("AdvanceDays mismatch: got {day:%d cash:%d}, want {day:%d cash:%d}",
			state.Day, state.Cash, ref.Day, ref.Cash)
	}
}
